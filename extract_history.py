...
def get_stock_history(symbol, save_path=STOCK_HISTORY_SAVEPATH, 
    ...
    try:
        ...
        html_table=soup.find("table", {"class":"table yf-1jecxey noDl"} )
        ...
        print(f"Error : finding {symbol} symbol data")


def get_stock_history_parallel(symbol_list, save_path=STOCK_HISTORY_SAVEPATH, 
                        epoch_start=datetime(2000,1,1).timestamp(), epoch_end= datetime.now().timestamp()
                        ):
    Path(save_path).mkdir(parents=True, exist_ok=True)
    
    pool = multiprocessing.Pool() 
    pool = multiprocessing.Pool(processes=PARALLEL_CORE_COUNT) 
